...
def main():
    ...
    while True:
        ...
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute("select id, host::text as host, port, auth_username, auth_password, latency_threshold_ms from proxies where is_enabled=true order by coalesce(last_checked_at, first_seen_at) asc nulls first limit %s", (batch_size,))
            batch = cur.fetchall()
            if not batch:
