...
async def get_session(session_id: str, db: Session = Depends(get_db)):
    ...
    return {'id': str(session.id), 'status': session.status, 'account_id': session.account_id, 'assigned_proxy_id': session.assigned_proxy_id, 'sticky_proxy_id': session.sticky_proxy_id, 'bytes_in': session.bytes_in, 'bytes_out': session.bytes_out, 'connections_count': session.connections_count, 'active_connections_count': session.active_connections_count}
...
async def get_session_connections(session_id: str, db: Session = Depends(get_db)):
    ...
    return {'items': [{'id': str(x.id), 'proxy_id': x.proxy_id, 'target_host': x.target_host, 'target_port': x.target_port, 'state': x.state, 'bytes_in': x.bytes_in, 'bytes_out': x.bytes_out, 'started_at': x.started_at, 'ended_at': x.ended_at} for x in items]}
